...
2. **割り当て** — Hub が空き Worker を選び、WebSocket で送る（満杯なら `503`）。割り当ての選定基準や、取りこぼした `queued` ジョブを救う仕組みは [ジョブ分配と負荷分散](dispatch.html) を参照。
...
- 空き Worker の選び方（実行中本数で並べてランダム同点解消）、`queued` のまま残ったジョブを救う **再配置ループ**、消えた Worker のジョブを整える **整合化処理** は [ジョブ分配と負荷分散](dispatch.html) で詳しく解説。
...
- 通知・更新・所有 Hub の取り扱い（Redis スキーマと有効期限）は [ジョブ分配と負荷分散 → Worker レジストリ](dispatch.html#レイヤ-3-worker-レジストリ-redis-スキーマ) を参照。
...
- 振り分けルールの詳細・`worker_id` の安定化・所有 Hub の管理は [ジョブ分配と負荷分散 → ネットワーク経路](dispatch.html#レイヤ-1-ネットワーク経路-nginx) を参照。
...
| ページ | 内容 |
|---|---|
| [ジョブ分配と負荷分散](dispatch.html) | 空き Worker の選び方、滞留ジョブの再配置ループ、取り残されたジョブの整合化処理、複数 Hub 構成での経路設計 |
